...
from faunacodec.cli._common import ROOT
from faunacodec.compression.dual_stream_dcvc import encode_roi_bg_dcvc
from faunacodec.video_io import VideoInfo


class TestDualStreamDcvcMetadata(unittest.TestCase):
    def test_uses_encoder_frame_index_map_authoritatively(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            with patch("faunacodec.compression.dual_stream_dcvc.probe_video", return_value=VideoInfo(32, 24, 30.0, 10)):
                with patch("faunacodec.compression.dual_stream_dcvc._capture_rendered_kept_frames_single_pass", return_value=fake_cache):
                    with patch(
                        "faunacodec.compression.dual_stream_dcvc.encode_dcvc_frames_to_bytes",
                        side_effect=[fake_roi, fake_bg],
                    ):
                        out = encode_roi_bg_dcvc(
    ...
    def test_rejects_inconsistent_encoder_frame_index_map(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            tmp = Path(td)
            video_path = tmp / "video.mp4"
            model_i = tmp / "i.pth.tar"
    ...
    def test_reads_source_video_once_for_roi_and_bg_streams(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            tmp = Path(td)
            video_path = tmp / "video.mp4"
            model_i = tmp / "i.pth.tar"
            ...
            with patch("faunacodec.compression.dual_stream_dcvc.probe_video", return_value=VideoInfo(6, 4, 30.0, 3)):
                with patch("faunacodec.compression.dual_stream_dcvc.cv2.VideoCapture", side_effect=lambda *_: _FakeCapture(frames)):
                    with patch("faunacodec.compression.dual_stream_dcvc.encode_dcvc_frames_to_bytes", side_effect=_fake_encode):
                        out = encode_roi_bg_dcvc(
                            source_video_path=video_path,
                            roi_bbox_map={},
