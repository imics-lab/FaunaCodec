...
from faunacodec.selection import remove_redundant_frames, validate_frame_selection_config
from faunacodec.selection.motion import parse_box_any
...
class TestFrameRemovalRuntime(unittest.TestCase):
    ...
    def test_validate_frame_selection_config_rejects_string_booleans(self) -> None:
        with self.assertRaisesRegex(ValueError, "frame_selection.params.roi.gray"):
            validate_frame_selection_config({"params": {"roi": {"gray": "false"}}})

        with self.assertRaisesRegex(ValueError, "frame_selection.params.bbox_smoothing.enable"):
            validate_frame_selection_config({"params": {"bbox_smoothing": {"enable": "false"}}})

        with self.assertRaisesRegex(ValueError, "frame_selection.dual_timeline.enable"):
            validate_frame_selection_config({"dual_timeline": {"enable": "true"}})
