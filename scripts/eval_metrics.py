...
def _load_frames(source: str, max_frames: int | None) -> list[np.ndarray]:
    p = Path(source)
    if p.is_dir():
        paths = sorted(
...
class _LPIPSCalc:
    ...
    def __call__(self, pred: np.ndarray, gt: np.ndarray) -> float | None:
        if not self._init():
            return None
        import torch
...
def _parse_vmaf_json(log_path: str) -> float | None:
