...
def _compress_decompress(video_path: Path, cfg: dict[str, Any],
                         decompression_cfg: dict[str, Any]) -> list[np.ndarray]:
    """Round-trip a video through the codec, returning the degraded BGR frames.

    These are the training inputs: whatever the decoder produces is exactly what
    RestoreUNet has to clean up at inference time.
    """
    from faunacodec import stages

    LOGGER.info(f"  Compressing {video_path.name} ...")
    archive = stages.compress(video_path, cfg, root_dir=ROOT)

    LOGGER.info("  Decompressing ...")
    # Encode and decode must agree on the DCVC checkpoints, or the streams will not
    # round-trip; take them from the compression config rather than duplicating them.
    decompression_cfg = copy.deepcopy(decompression_cfg)
    dcvc = (cfg.get("compression", {}) or {}).get("dcvc", {})
    if dcvc:
        decompression_cfg.setdefault("decompression", {})["dcvc"] = dcvc

    return stages.decompress(archive, decompression_cfg).frames


def _extract_original_frames(video_path: Path, resolution: tuple | None = None) -> list[np.ndarray]:
...
def _resize_video_temp(video_path: Path, resolution: tuple | None, max_frames: int | None = None) -> Path:
...
def process_video(
    ...
    output_dir: Path,
    cfg: dict[str, Any],
    decompression_cfg: dict[str, Any],
    ...
    degraded_frames = _compress_decompress(compress_path, cfg, decompression_cfg)
...
def _parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--config",     default="configs/compression.yaml")
    p.add_argument("--decompression-config", default="configs/decompression.yaml")
...
def main() -> int:
    ...
    decompression_cfg = _load_config(args.decompression_config)
    ...
    for i, vp in enumerate(videos, 1):
        ...
        try:
            n = process_video(vp, output_dir, cfg, decompression_cfg,
