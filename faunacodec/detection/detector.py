...
def _union_merge_boxes(
    ...
    iou_thr: float,
) -> tuple[list[tuple[float, float, float, float]], list[float] | None]:
    ...
    out_scores: list[float] | None = [] if scores is not None else None
...
def _detector_quantize(runtime: dict[str, Any]) -> int | str | None:
    """Resolve current precision config while accepting pre-Ultralytics-8.4 configs."""
    if "quantize" in runtime:
        return runtime["quantize"]
    if "half" in runtime:
        return 16 if bool(runtime["half"]) else None
    return None


def _seed_klt_points(gray: np.ndarray, bbox: tuple[float, float, float, float], max_pts: int) -> np.ndarray | None:
    h, w = gray.shape[:2]
...
def _klt_translation(
    ...
    pts_prev: np.ndarray,
    min_valid: int,
) -> tuple[np.ndarray | None, tuple[float, float] | None, int]:
...
def _detect_proc_boxes(
    ...
    boxes: list[tuple[float, float, float, float]] = []
    scores: list[float] = []
    for (x1, y1, x2, y2), sc, c in zip(xyxy, confs, clss, strict=False):
...
def _select_detector_weights(paths: dict[str, Any], runtime: dict[str, Any]) -> Path:
    """Choose between the PyTorch and ONNX detector weights.

    ONNX Runtime is usually faster on the edge device, but it is an optional
    dependency, so fall back to the PyTorch weights unless prefer_onnx_strict says the
    fallback should be an error instead.
    """
    torch_weights = resolve_repo_path(paths["animal_model_path"])
    if not bool(runtime.get("prefer_onnx", False)):
        return torch_weights

    strict = bool(runtime.get("prefer_onnx_strict", False))
    onnx_raw = paths.get("animal_model_path_onnx")
    if not onnx_raw:
        if strict:
            raise ValueError(
                "roi_detection.runtime.prefer_onnx_strict is set but "
                "roi_detection.paths.animal_model_path_onnx is missing"
            )
        return torch_weights
    ...
    try:
        ...
        if strict:
            raise RuntimeError(
                "roi_detection.runtime.prefer_onnx_strict is set but onnxruntime is "
                'not installed: pip install -e ".[onnx]"'
            ) from exc
        return torch_weights

    onnx_weights = resolve_repo_path(onnx_raw)
    if not onnx_weights.exists():
        if strict:
            raise FileNotFoundError(f"ONNX detector weights not found: {onnx_weights}")
        return torch_weights
    return onnx_weights
...
def run_roi_detection(
    ...
    if not paths.get("animal_model_path"):
        raise ValueError("config['paths']['animal_model_path'] is required")
    model_path = _select_detector_weights(paths, rt)
    ...
    # Accept old FaunaCodec configs without forwarding Ultralytics' deprecated
    # ``half`` keyword.
    quantize = _detector_quantize(rt)
    ...
    tracks: list[_Track] = []
    next_id = 1
    prev_gray_p: np.ndarray | None = None
