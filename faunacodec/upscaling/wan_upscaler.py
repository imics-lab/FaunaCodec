...
class WanUpscaler:
    ...
    def upscale_sequence(self, frames: list[np.ndarray],
                         detections: dict | None = None) -> list[np.ndarray]:
        """
        Upscale a list of BGR uint8 frames.
        Returns a list of BGR uint8 frames at (out_h, out_w).
        detections is accepted for API compatibility; Wan conditions on the first
        frame of each chunk and has no per-frame ROI overlay.
        """
        N = len(frames)
        if N == 0:
            return []
