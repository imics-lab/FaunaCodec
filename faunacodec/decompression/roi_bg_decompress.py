...
def _remap_container_path(raw_path: str) -> Path | None:
    """
...
def _resolve_runtime_path(raw_path: str, kind: str) -> Path:
    ...
    if kind == "repo":
        candidates.append((REPO_ROOT / "third_party" / "dcvc").resolve())
        candidates.append((REPO_ROOT / "DCVC").resolve())
    elif kind in {"model_i", "model_p"}:
        candidates.append((REPO_ROOT / "models" / p.name).resolve())
...
def _resolve_hint(meta_hint: Any, user_limit: int | None) -> int | None:
    hint: int | None = None
...
def _normalize_cuda_index(cuda_idx: Any) -> int | None:
...
def _decode_stream_bytes_dcvc(
    ...
    frame_consumer: Callable[[np.ndarray], None] | None = None,
) -> list[np.ndarray]:
    """
    ...
    repo_dir = str(
        _resolve_runtime_path(
            str(dcvc_cfg.get("repo_dir", "third_party/dcvc")), kind="repo"
        )
    )
...
def decode_roi_bg_streams(
    ...
    bg_bin_bytes: bytes,
    meta: dict[str, Any],
    progress_cb_roi: Callable[[int], None] | None = None,
    progress_cb_bg: Callable[[int], None] | None = None,
    max_frames_roi: int | None = None,
    max_frames_bg: int | None = None,
