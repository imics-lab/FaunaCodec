...
from ..archive import read_payloads
from ..config import coerce_bool
from ..masking import build_frame_mask, compose_soft, mask_to_alpha
...
def _normalize_cuda_device(raw_device: Any, *, field_name: str) -> tuple[str, int | None]:
...
def _enforce_strict_gpu_runtime(dec_cfg: dict[str, Any], meta: dict[str, Any]) -> None:
    ...
    if not coerce_bool(dcvc_cfg.get("use_cuda", True), default=True):
    ...
    if bool(interp_cfg.get("enable", True)):
        interp_device, _ = _normalize_cuda_device(
            interp_cfg.get("device", "cuda"),
            field_name="decompression.interpolate.device",
        )
        interp_cfg["device"] = str(interp_device)
...
def _filter_anchor_positions(
    ...
    out_i: list[int] = []
    out_p: list[int] = []
    for idx, pos in zip(indices, positions, strict=False):
        i = int(idx)
...
def _frame_motion_urgency(frame_drop_json: dict[str, Any], frame_idx: int) -> float:
    ...
    sparse = frame_drop_json.get("motion_urgency_by_frame", {}) or {}
    if isinstance(sparse, dict) and "motion_urgency_by_frame" in frame_drop_json:
        try:
            return _clamp01(float(sparse.get(frame_key, 0.0)))
        except (TypeError, ValueError):
            return 0.0

    pf_all = frame_drop_json.get("per_frame", {}) or {}
    pf = pf_all.get(frame_key, {}) or {}
...
def _anchor_context_at(
    *, slot: int,
    ...
    anchor = compose_soft(roi_frame, bg_frame, alpha)
...
def _build_roi_segment(
    *, left_slot: int, right_slot: int,
    ...
    interpolator: AmtInterpolator | None,
...
def decompress_archive(
    ...
    *,
    ...
    roi_alpha_still: float | None = None,
    roi_alpha_motion: float | None = None,
    roi_mask_dilate: int | None = None,
    roi_stabilize_overlap_only: bool = False,
    amt_batch_size: int | None = None,
    amt_crop_margin: int | None = None,
    max_frames: int = 0,
    lossless_output: Path | None = None,
    lossless_yuv420_output: Path | None = None,
    ...
    payloads = read_payloads(archive_path)
    ...
    roi_decode_limit: int | None = None
    bg_decode_limit: int | None = None
    if int(max_frames) > 0:
    ...
    writer = None
    lossless_writer: _LosslessEvalWriter | None = None
    lossless_yuv420_writer: _LosslessYuv420Writer | None = None
    decode_tmp_dir = Path(tempfile.mkdtemp(prefix="decomp_decode_"))
    roi_store = bg_store = None
    try:
        ...
        roi_interpolator: AmtInterpolator | None = None
        roi_mode = "linear"
        if interpolation_requested:
            roi_interpolator = rd._init_amt_interpolator(interp_cfg)
            roi_mode = "amt"
            ...
            if int(np.count_nonzero(mask)) == 0:
                ...
                if slot is not None:
                    ...
                    if right_slot == left_slot or t <= li or t >= ri:
                        ...
                        out = compose_soft(roi_source, bg_f, alpha)

            dilated_mask = cv2.dilate(mask, stab_kernel, iterations=1) \
                if stab_kernel is not None else mask

            if roi_temporal_stabilize and prev_out is not None:
                stab_mask = dilated_mask
