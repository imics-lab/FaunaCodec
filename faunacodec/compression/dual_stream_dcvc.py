...
from ..masking import boxes_for_frame, build_boxes_mask
from ..paths import resolve_repo_path
from ..video_io import VideoInfo, probe_video
from .dcvc_encoder import encode_dcvc_frames_to_bytes
...
def _iter_kept_roi_frames(
    *,
    ...
    """
    roi_keep_set = {int(x) for x in roi_kept_frames}
        ...
        while True:
            ...
            if src_idx in roi_keep_set:
                yield int(src_idx), _apply_roi_mask(
                    frame,
                    boxes_for_frame(roi_bbox_map, src_idx),
                    roi_min_conf=float(roi_min_conf),
                )
            src_idx += 1
    finally:
...
def _iter_kept_bg_frames(
    *,
    ...
    """
    bg_keep_set = {int(x) for x in bg_kept_frames}
    cap = cv2.VideoCapture(str(video_path))
    if not cap.isOpened():
...
def encode_roi_bg_dcvc(
    *,
    ...
    source_video = resolve_repo_path(source_video_path, root)
    ...
    repo_dir = resolve_repo_path(
        str(dcvc_cfg_raw.get("repo_dir", "third_party/dcvc")), root
    )
    model_i = resolve_repo_path(str(dcvc_cfg_raw.get("model_i", "")), root)
    model_p = resolve_repo_path(str(dcvc_cfg_raw.get("model_p", "")), root)
    ...
    if low_memory:
        ...
        with tempfile.TemporaryDirectory(prefix="faunacodec_dual_") as td:
    ...
    meta: dict[str, Any] = {
        "video": {
            ...
            "fps": float(info.fps),
            "frames_total": int(info.frame_count),
            "start_frame": 0,
            "end_frame": int(max(0, info.frame_count - 1)),
        },
        "roi": {
