...
def _parse_frame_selection_config(cfg: dict[str, Any]) -> dict[str, Any]:
    params = _as_section(cfg.get("params", {}), "frame_selection.params")

    p_roi = _as_section(params.get("roi", {}), "frame_selection.params.roi")
    ...
    to_gray = _as_bool(
        p_roi.get("gray", True),
        "frame_selection.params.roi.gray",
        True,
    )

    p_smooth = _as_section(
        params.get("bbox_smoothing", {}),
        "frame_selection.params.bbox_smoothing",
    )
    smooth_enable = _as_bool(
        p_smooth.get("enable", True),
        "frame_selection.params.bbox_smoothing.enable",
        True,
    )
    ...
    p_motion = _as_section(params.get("motion", {}), "frame_selection.params.motion")
    pixel_t_low = _as_float_range(
        p_motion.get("t_low", 0.9),
        "frame_selection.params.motion.t_low",
        minimum=0.0,
    )
    pixel_t_high = _as_float_range(
        p_motion.get("t_high", 1.9),
        "frame_selection.params.motion.t_high",
        minimum=0.0,
    )
    ...
    dual_cfg = _as_section(cfg.get("dual_timeline", {}), "frame_selection.dual_timeline")
    dual_enabled = _as_bool(
        dual_cfg.get("enable", True),
        "frame_selection.dual_timeline.enable",
        True,
    )
    roi_motion_interval = _as_int_ge(
        dual_cfg.get("roi_motion_interval", 2),
        "frame_selection.dual_timeline.roi_motion_interval",
        1,
    )
    roi_still_interval = _as_int_ge(
        dual_cfg.get("roi_still_interval", 3),
        "frame_selection.dual_timeline.roi_still_interval",
        1,
    )
    bg_interval = _as_int_ge(
        dual_cfg.get("bg_interval", 6),
        "frame_selection.dual_timeline.bg_interval",
        1,
    )
    bg_idle_interval = _as_int_ge(
        dual_cfg.get("bg_idle_interval", 10),
        "frame_selection.dual_timeline.bg_idle_interval",
        1,
    )
...
def validate_frame_selection_config(cfg: dict[str, Any]) -> None:
    _parse_frame_selection_config(cfg)


def parse_box_any(obj: Any) -> BoxXYXY | None:
    if obj is None:
        return None
...
def ema_smooth(
    ...
    return tuple((a * p + (1.0 - a) * c) for p, c in zip(prev, curr, strict=False))  # type: ignore[return-value]


def union_boxes_xyxy(boxes: list[tuple[float, float, float, float]]) -> tuple[float, float, float, float] | None:
    if not boxes:
        return None
...
def _track_id_from_raw(raw: Any) -> int | None:
...
def remove_redundant_frames(video_path: str, bbox_map_raw: dict[int, list[Any]], cfg: dict[str, Any]) -> dict[str, Any]:
    runtime_cfg = _parse_frame_selection_config(cfg)
